...
Readest uploads state to S3. S3 sends object-created notifications to SQS. KEDA scales a Kubernetes `ScaledJob` from the queue, and the `readest-obsidian-sync run` worker fetches the changed Readest config and `library.json`, renders a deterministic book note, writes it to the mounted vault, and runs `ob sync`.
...
The multi-stage image contains the statically linked worker and pinned `obsidian-headless@0.0.14` CLI, and runs as the non-root `node` user. The intended entrypoint is `readest-obsidian-sync run`.

## Helm

The reusable chart at `charts/readest-obsidian-sync` renders a KEDA `ScaledJob`, ServiceAccount, and a chart-created PVC by default. It intentionally does not render a Service or Ingress. Defaults use one maximum replica because concurrent writes to one vault are unsafe.
...
The deployment repository supplies `aws.region`, `aws.sqsQueueUrl`, `aws.s3Bucket`, the image tag, and optionally `existingSecret`. An existing Secret must contain `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`. Set `vault.existingClaim` instead of `vault.createPvc` when storage is managed elsewhere. The `obsidian-setup` init container waits for `ob sync-status` to succeed, so the worker is blocked until the vault has been configured. Bootstrap a new vault by finding the pending Job pod and running `kubectl exec -it <pod> -c obsidian-setup -- ob login`, followed by `kubectl exec -it <pod> -c obsidian-setup -- ob sync-setup --path /var/lib/obsidian-vault`. Once the setup files exist on the PVC, subsequent Job starts pass the init check automatically. No credentials or AWS account IDs are embedded in the chart.
